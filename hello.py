...
def get_wordcount(sentences):
    return {(len(sentence.split())): sentence for sentence in sentences}

def longest_sentence(string):
    sentences = sentence_processor(string)
    wordcounts = get_wordcount(sentences)

    print(wordcounts[max(wordcounts)])
    print()
    print(f'The longest sentences has {max(wordcounts)} words')
